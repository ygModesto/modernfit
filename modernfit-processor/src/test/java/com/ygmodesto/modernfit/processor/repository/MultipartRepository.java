...
public interface MultipartRepository {
  ...
  @Multipart
  @POST("/user/create")
  User createUser(@Part User user);

  @Multipart
  @POST("/users/createHeavy")
  Collection<User> createUsers(
  ...
  User updateUserAttacheds(@Path Long id, @PartMap Map<String, TypedContent> attacheds);

  @Multipart
  @PUT("/user/update/{id}")
  User updateUser(@Path Long id, @Part User user, @Part File file);
